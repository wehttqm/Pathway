...
def _get_program_areas(html: bytes):
    ...
    inner_div = div.find('div', class_=re.compile('w3-row field field--name-body field--type-text-with-summary field--label-hidden w3-bar-item field__item'))
    ...
    for table in tables:
        ...
        for row in rows:
            ...
            if text not in _names:
                program_areas.append({"href": a['href'], "name": text})
                _names.append(text)
